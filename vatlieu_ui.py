...
class VatLieuFrame(tk.Frame):
    ...
    def create_ui(self):
        ...
        tk.Button(frame_btn, text="Xuất Excel", bg="#1818EF",
                  command=self.export_excel, **btn).grid(row=0, column=5, padx=5)
    ...
    def export_excel(self):
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT v.MaVL, v.TenVL, v.DonViTinh, v.GiaNhap, v.GiaBan,
                   v.TonKho, n.TenNCC
            FROM vatlieu v
            LEFT JOIN nhacungcap n ON v.MaNCC = n.MaNCC
        """)
        ...
        df = pd.DataFrame(rows, columns=[
            "Mã VL", "Tên VL", "Đơn vị tính",
            "Giá nhập", "Giá bán", "Tồn kho", "Nhà cung cấp"
        ])
